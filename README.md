...
当前也补上了最小 workspace auth 壳层：`/login` 已收口到统一认证入口，默认本地环境使用开箱即用的 `builtin` 账号密码 provider，也可以切到 `ZITADEL` provider 继续使用同源账号密码登录与 OIDC 跳转登录；`/workspace` 提供 Dify 风格的应用工作台入口，`/admin/members` 提供成员新增与角色配置；应用入口仍然回到 7Flows 现有的 xyflow 编排页，而不是分叉成第二套执行心智。
...
node scripts/kill-stale-duplicate-processes.js --kill
```
...
- 默认密码：`admin123`
- 认证方式：统一走 `/api/auth/*`；本地默认 provider 是 `builtin`，需要外部身份时可切到 `ZITADEL`
- 应用工作台：`/workspace`
- 成员管理：`/admin/members`
