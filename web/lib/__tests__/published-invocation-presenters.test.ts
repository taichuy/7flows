...
describe("published invocation presenters", () => {
  it("集中生成 publish invocation detail helper copy", () => {
    expect(
      ...
      legacyAuthGovernanceDescription: expect.stringContaining("workflow 级 legacy publish auth artifact"),
  ...
  it("允许 publish sampled run handoff 复用显式 workflow detail scope", () => {
    const sample = listPublishedInvocationRunFollowUpSampleViews({
      affected_run_count: 1,
      sampled_run_count: 1,
      waiting_run_count: 1,
  ...
  it("允许 sampled run view 复用显式 workflow detail scope", () => {
    const sample = resolvePublishedInvocationRunFollowUpSampleView(
      {
        workflow_id: "workflow-1",
        run_id: "run-1",
