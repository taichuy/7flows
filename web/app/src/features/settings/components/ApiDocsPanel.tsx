...
export function ApiDocsPanel() {
  ...
  const selectedCategoryId =
    categories.find((category) => category.id === requestedCategoryId)?.id ??
    null;
  ...
  const categoryOperationsQuery = useQuery({
    queryKey: settingsApiDocsCategoryOperationsQueryKey(
      selectedCategoryId ?? ''
    ),
  ...
  const selectedOperationId =
    operations?.find((operation) => operation.id === requestedOperationId)
      ?.id ?? null;
        ...
        <div className="api-docs-panel__header-control">
          ...
          <Select
            ...
            filterOption={(input, option) =>
              ...
              ).includes(normalizeApiDocsCategorySearchText(input))
            }
  ...
  return (
    ...
    >
      <div className="api-docs-panel">{content}</div>
    </SettingsSectionSurface>
  );
}
