...
import type { WorkspaceAppsWorkbenchProps } from "@/components/workspace-apps-workbench/shared";
...
function buildApp(index: number) {
  return {
    id: `workflow-${index}`,
    name: `Workflow ${index}`,
    href: `/workflows/workflow-${index}`,
    status: index % 2 === 0 ? "published" : "draft",
    healthLabel: index % 2 === 0 ? "可调用" : "可继续编排",
    recommendedNextStep: `继续处理 workflow ${index}`,
    updatedAt: `2026-03-${String(index).padStart(2, "0")}T12:00:00Z`,
    nodeCount: index + 2,
    publishCount: index % 2,
    missingToolCount: 0,
    followUpCount: index % 3 === 0 ? 1 : 0,
    mode: { label: index % 2 === 0 ? "Agent" : "ChatFlow", shortLabel: "App" },
    track: {
      id: "应用新建编排",
      priority: "P0",
      focus: "工作台入口",
...
function buildWorkbenchProps(
  overrides: Partial<WorkspaceAppsWorkbenchProps> = {}
  ...
  return {
    ...
    filteredApps: [buildApp(1), { ...buildApp(2), name: "ChatFlow Alpha" }],
    searchState: {
      filter: "draft",
      mode: null,
      track: null,
...
describe("WorkspaceAppsWorkbench", () => {
  it("renders a two-level workspace shell with modal create entry and card actions", () => {
  ...
  it("keeps the directory paginated when the workspace has many apps", () => {
    const html = renderToStaticMarkup(
      createElement(
        WorkspaceAppsWorkbench,
        buildWorkbenchProps({
          visibleAppSummary: "全部 7 个应用",
          filteredApps: Array.from({ length: 7 }, (_, index) => buildApp(index + 1))
        })
      )
    );
