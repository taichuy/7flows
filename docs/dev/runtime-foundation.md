...
- 2026-03-16 compat adapter 的 invoke payload 已继续补齐 execution forwarding：`ToolGateway.execute()` 现在会把 `ResolvedExecutionPolicy.as_runtime_payload()` 连同 `traceId` 一起传给 `PluginCallProxy`，后者会把同一份 `execution` payload 透传到 compat adapter `/invoke` 请求；因此 `llm_agent` / tool node 上已经进入 trace 的 execution class、profile、timeout/network/filesystem policy，不再只停留在 7Flows 内部事件与 artifact 里，compat adapter 侧也能拿到统一的执行提示继续演进真实隔离边界。
...
- 2026-03-16 sensitive access 的通知投递主链已从“创建即失败”升级为真实异步语义：`in_app` 通道继续即时记为 `delivered`，而 `webhook / slack / feishu / email` 现在会先创建 `pending` 的 `NotificationDispatchRecord`，再通过新的 transaction-aware `NotificationDispatchScheduler` 在 `after_commit` 后派发 `notifications.deliver_dispatch` worker；`webhook` 已具备最小真实 HTTP 投递能力，`slack / feishu` 支持直接使用 webhook URL 作为 target，`email` 仍会通过 adapter 明确失败而不是把失败语义写死在 request path。`/api/sensitive-access/notification-dispatches/{dispatch_id}/retry` 与 inbox 内的“重试最新通知”动作也已复用同一条 pending -> worker -> delivered/failed 主链。
...
- 2026-03-16 最新复核结果：后端基础框架仍足够继续推进主业务闭环，且本轮已把敏感访问通知从“request path 直接失败”收成最小可用的 pending + worker/adapter 语义；最新验证已覆盖 `api/.venv/Scripts/uv.exe run pytest -q`（`278 passed`），以及 `api/.venv/Scripts/uv.exe run ruff check app/services/notification_dispatch_scheduler.py app/services/notification_delivery.py app/tasks/notifications.py app/services/sensitive_access_control.py tests/test_notification_dispatch_scheduler.py tests/test_notification_delivery.py tests/test_sensitive_access_routes.py tests/test_celery_app.py`。当前稳定性基线继续提升，但项目仍未进入“只剩人工逐项界面设计 / 人工验收”的阶段。
...
- Workflow library、system overview、plugin adapters、runtime activity、credentials API 已具备；`sensitive-access` 资源注册、访问请求、审批票据，以及通知投递查询 / 决策 / 重试 API 已接到 runtime 与 trace export 主链，为后续 approval inbox、published surface export 治理和通知 worker 预留稳定事实入口。
...
- 当前已经有 approval-specific route、notification delivery query / retry API 与最小 operator inbox；后续若继续补统一敏感访问控制，应优先沿这套事实层扩 `notification worker / adapter`、approval timeline 与 operator 控制面，而不是另起一套状态体系。
...
1. **P0：继续把 graded execution 从 execution-aware 扩成真实隔离能力**
   - `RuntimeExecutionAdapterRegistry`、`sandbox_code` host-subprocess MVP，以及 ToolGateway / `llm_agent.toolPolicy.execution` / tool node 的 execution-aware dispatch 已落地；2026-03-16 先后补齐了 `llm_agent` 单次 tool call 对 `toolPolicy.execution` 的覆盖语义，以及 compat adapter `/invoke` payload 对统一 `execution` contract 的透传。下一步优先补真实 `sandbox` / `microvm` tool adapter、compat plugin 对 forwarded execution payload 的实际隔离兑现，以及 execution trace 摘要聚合，避免 execution policy 长期停留在“有 trace/contract，但少数能力仍 fallback”。
2. **P0：继续扩统一敏感访问控制闭环**
