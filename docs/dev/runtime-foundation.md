...
- 2026-03-16 sensitive access 的通知投递语义已进一步补齐“诚实性”：`in_app` 通道继续即时记为 `delivered`，而 `slack / email / webhook / feishu` 等尚未接入 adapter/worker 的通道不再伪装成长期 `pending`，而是会直接记为 `failed` 并写出明确原因；`/api/sensitive-access/notification-dispatches/{dispatch_id}/retry` 与 inbox 内的“重试最新通知”动作也已落地，为后续真实通知 worker / adapter 接入保留同一条事实链与 operator 入口。
...
- 2026-03-15 最新复核结果：后端基础框架仍足够继续推进主业务闭环，且本轮已把 credential `allow_masked` 从“事实等同 allow”收成最小可用的 masked-handle 语义；最新验证已覆盖 `api/.venv/Scripts/uv.exe run pytest -q`（`252 passed`）、`api/.venv/Scripts/uv.exe run ruff check app/api/routes/published_endpoint_invocation_support.py app/schemas/workflow_publish.py tests/test_workflow_publish_routes.py tests/test_published_invocation_detail_access.py tests/test_published_native_async_routes.py tests/test_published_protocol_async_routes.py`、`web/pnpm lint` 与 `web/pnpm exec tsc --noEmit`。当前稳定性基线继续提升，但项目仍未进入“只剩人工逐项界面设计 / 人工验收”的阶段。
