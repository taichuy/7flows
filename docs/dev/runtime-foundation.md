...
6. **P1：继续提高工作流编辑器完整度**
   - 在现有 `runtimePolicy.execution / retry / join`、节点 contract、workflow `variables`、workflow `publish` draft、node catalog `support_status / support_summary`、前端保存前 capability validation、后端 workflow / workspace starter persistence guard、后端 node/publish contract schema validation 与 `llm_agent.toolPolicy` 基础上，继续补敏感访问策略入口、schema builder、更细粒度的 binding existence / reference 校验，以及更清晰的 advanced JSON / structured form 边界。
