...
- `sensitivity_level` 驱动的统一敏感访问控制、人工审核与通知闭环已确定为架构初期事项；在上一轮 `SensitiveResourceRecord / SensitiveAccessRequestRecord / ApprovalTicketRecord / NotificationDispatchRecord` 事实层与 `/api/sensitive-access/*` API 基础上，本轮继续把它从 runtime `credential resolve` 扩到 `mcp_query / authorized_context` 的 context read 主链：当 node/agent 解析 `credential://...` 或读取命中 `workflow_context` 资源的上游 artifact 时，运行态现在都会创建/复用访问请求、在审批未完成时进入 `waiting_tool`，并在审批 `approved / rejected` 后通过既有 `RunResumeScheduler` 回到 `waiting/resume` 主链。对 `L2` context read，当前还会返回结构化 masked payload，而不是继续把原始内容直接透给下游。当前仍缺 ToolGateway 与 publish export 的真实拦截挂点，以及更完整的通知 worker / inbox 落地；`allow_masked` 在 credential path 上也还没有真正区分“masked vs 明文”。
- 2026-03-15 最新复核结果：后端 `api/.venv/Scripts/uv.exe run pytest -q` 继续通过（`235 passed`）；除敏感访问控制的 runtime credential gating 外，本轮还补齐了 `mcp_query / authorized_context` 的 sensitive context gating（含 `allow_masked` 与 `require_approval -> resume`），以及 direct `tool` node 对 normalized `ToolExecutionResult(status="waiting")` 的 suspend / schedule / resume 处理，避免 direct tool node、context read 和 llm_agent tool call 的 waiting 语义继续分叉。changed-files `ruff check` 与 `pytest -q tests/test_runtime_credential_integration.py`（`12 passed`）已通过。后端全量 `ruff check` 的历史风格/整理债务仍未在本轮整体清零，因此稳定性基线继续提升，但还未达到“全仓库零告警”。
...
- `api/app/services/runtime_node_dispatch_support.py`：当前已同时承接 direct tool node、credential gating、sensitive context read gating 与 waiting result suspend 处理；职责仍清晰，但后续若继续补 ToolGateway 拦截、publish export policy 或更多 node-type special case，优先考虑把 tool/context 两支再拆到 helper，而不是让 dispatch support 演进成新的 God file。
- `api/app/services/sensitive_access_control.py`：目前同时覆盖 resource/access request/ticket/notification 查询、默认策略与审批恢复，已经接近新的治理热点；若继续补 notification worker、更多 source matcher 或 policy plug-in，建议优先拆 query/mutation/policy helper，避免 service 主文件继续线性膨胀。
...
2. **P0：继续扩统一敏感访问控制闭环**
   - `SensitiveAccessRequest`、`ApprovalTicket`、`NotificationDispatch`、`/api/sensitive-access/*`、runtime `credential resolve`、`mcp_query / authorized_context` 的 context read 拦截，以及审批后的 `waiting/resume` 调度都已落地；下一步优先把同一套控制继续挂到 ToolGateway 与 publish export 入口，并补真实通知 worker / inbox，同时把 credential path 的 `allow_masked` 从“事实等同 allow”收成真正的 masked/handle 语义，而不是只停留在 credential path + context read 两条局部主链。
