...
- issue sync step 现在也会把 `tracking_issue_action`、`tracking_issue_number`、`tracking_issue_url`、`tracking_issue_should_track`、`tracking_issue_current_ref` 与 `tracking_issue_default_branch` 写入 `GITHUB_OUTPUT` 并透传为 job outputs，后续 workflow / agent 不需要再翻 issue 列表或解析日志，就能直接拿到这条外部 blocker handoff 的机器可读坐标。
