...
- 当 `dependency_graph_check_error` / `dependencyGraphVisibility.checkError` 明确命中 GitHub API `rate limit` 时，推荐动作现在会切到 `rerun_with_authenticated_github_api`，优先引导 workflow maintainer 换成更高配额的 token / `gh` 凭证后再重跑；不要把这类结果误判成 `actions: read` 丢失或仓库 `Dependency graph` 设置还没打开。
- 但如果同一份 `dependabot-drift.json` / `dependency-submission.json` 已经带回 `repositoryBlockerEvidence.kind=dependency_graph_disabled`，推荐动作会继续把 `enable_dependency_graph` 放在首位，把 `rerun_with_authenticated_github_api` 降成“补齐 graph visibility 证据”的后续动作；不要因为本地 `gh` 配额或凭证问题，就忽略 submission artifact 已经给出的仓库设置根因。
- 相反，只要 drift 已经拿到可用的 `dependencySubmissionEvidence.report`（例如已经知道 `repositoryBlockerEvidence` 或 `visibleRoots/missingRoots`），后续 handoff 就不再因为历史上的 `actions: read` 噪音额外插入 `grant_actions_read`，而是继续围绕真实 blocker / visibility 证据推进，避免 workflow / agent 被次要权限问题带偏优先级。
...
- 同一个 `sync_issue` step 现在还会在 workflow summary 末尾追加一段 `Security drift tracking issue` handoff：直接显示 issue sync action、tracking issue URL、`tracking_issue_state_changed` 对应的语义解释，以及首要动作是否 `manualOnly` / 对应官方文档链接，避免维护者还得翻 artifact 或 issue body 才能知道“本轮只是 freshness 刷新，还是 blocker 真的变了”。
...
- 当维护者手动或定时跑完 `Dependency Graph Submission` 后，`GitHub Security Drift` 现在会通过 `workflow_run` 自动接力复验同一提交的 drift 结果，不需要再额外手点第二次 drift；为避免 lockfile push 同时触发两条 workflow 时重复跑一遍，首次由 `push` 触发的 submission run 仍会跳过这条自动接力，但如果维护者对这条 `push` run 执行 GitHub UI `Re-run jobs`，新的 `run_attempt > 1` 会重新触发 drift 自动复验。自动接力时 workflow 也会显式 checkout 上游 `head_sha`，并把 `head_branch` 通过 `--current-ref-name` 透传给 issue sync，避免把 feature/manual rerun 的结果误写成默认分支 blocker。
- artifact 现在会同时保留：
