...
4. 当后续出现更强机器检索需求时，优先补充 run trace 相关接口，而不是继续把原始日志堆到首页。
...
1. 为 `run_events` 补机器检索接口时，保持“机器层增强，而不是 UI 泄洪”。
2. 回到 run 诊断页继续做交互时，只做人类排障效率提升，不把它变成 AI 唯一追溯入口。
3. 后续若补 trace / replay / export 能力，也按这三层边界继续展开。
