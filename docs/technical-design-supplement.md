...
> 文档分层约定：本文只保留持续有效的技术基线；开源 / 商业边界与版本分层见 `docs/open-source-commercial-strategy.md`；当前实现索引放在 `docs/dev/`；当前开发者自己的按日期过程留痕如需保留，请放在 `docs/.private/history/`；废弃文档统一归档到 `docs/expired/`。
