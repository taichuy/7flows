...
- 认证内核采用 `AuthKernel + AuthenticatorRegistry + SessionIssuer` 三层中间模型；P1 只内置 `password-local`，后续第三方登录优先以宿主内核中的 `hosted provider` 扩展，而不是直接进入当前 `Wasm plugin-runner` 主线。
...
- `AuthKernel`
  - 统一登录入口、禁用校验、`root` 保护、审计触发、用户主模型装配
- `AuthenticatorRegistry`
  - 注册并解析可用认证器
  - P1 启动时至少注册一个内置认证器：`password-local`
- `SessionIssuer`
  - 统一负责 Redis session、Cookie、CSRF、滑动续期、`session_version` 失效

扩展规则：
- 认证器不直接写 Cookie、CSRF、Redis 细节
- 认证器只负责“如何验证身份并返回哪个用户”
- 当前 P1 只落 `password-local`
- 后续如扩展 `OIDC`、企业微信、短信验证码等，优先作为 `hosted provider` 接入 `AuthenticatorRegistry`
- 这些认证扩展不进入当前 `Provider/Node/DataSource/PublishAdapter` 四类插件主线，也不先放进 `runner_wasm`
...
约束：
- `name` 全局唯一，例如 `password-local`
- P1 bootstrap 至少写入一条内置认证器记录：`password-local`
- `auth_type` 表示认证实现类型，P1 首轮只启用 `password-local`
- `options` 用于承接后续认证器公开配置，不把第三方认证字段直接塞进 `users`
- 当前控制台不提供认证器 CRUD 页面，但数据模型先保留扩展边界

### 4.11 用户认证标识表 `user_auth_identities`
...
约束：
- `password-local` 首轮不强制依赖该表完成登录，可先以 `users.account/email/phone` 为事实来源
- 该表用于给未来第三方认证保留“一个用户绑定多个外部身份”的稳定模型
- `(authenticator_name, subject_type, subject_value)` 必须唯一
- 后续新增第三方登录时，不需要重写 `users` 主表，只需新增身份绑定记录
...
5. 幂等 seed 权限目录
6. 幂等创建默认团队
7. 幂等创建内置角色模板 `root/admin/manager`
8. 幂等创建 `.env` 指定的默认 `root` 账号
9. 幂等创建 `root` 的应用级角色绑定与初始化团队 membership
...
- 登录请求可选携带 `authenticator`，默认取 `password-local`
...
- `POST /api/console/auth/login`
  - 输入：`authenticator?`、`identifier`、`password`
...
- 根据请求解析认证器
- 调用认证器校验身份
- 统一执行用户状态校验与 `root` 保护规则
- 触发登录审计
- 将认证结果交给 `session_issuer`

### 10.4 `authenticator_registry`

负责：
- 注册内置认证器
- 按 `authenticator` 名称解析实现
- 管理默认认证器 `password-local`
- 为后续 hosted 第三方认证扩展提供统一挂载点

### 10.5 `password_local_authenticator`

负责：
- 处理 `account/email/phone + password` 校验
- 只返回已验证用户，不直接写 session
- 复用 `users` 表中的登录标识与密码哈希
...
### 13.2 当前不把认证扩展直接做成通用代码插件

这是刻意边界，而不是能力缺失。

原因：
- 认证直接触碰 Cookie、CSRF、会话、账号绑定与密钥配置，安全边界高于普通业务插件
- 当前插件主线聚焦模型、节点、数据源、发布适配，不宜把认证一起塞进 `runner_wasm`
- 先把认证做成宿主内核中的 `provider registry`，已经足够支撑后续第三方登录扩展，同时能避免过早把高安全面能力外包给运行时插件

### 13.3 `default_display_role` 不做外键
