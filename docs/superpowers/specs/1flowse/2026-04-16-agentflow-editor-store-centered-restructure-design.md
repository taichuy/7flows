# 1Flowse agentFlow Editor Store-Centered 重构设计稿
...
- 参考 Dify 后，1Flowse 应该收口成什么样的前端架构
...
- document schema 继续以 `@1flowse/flow-schema` 为唯一真值，不引入第二套编辑器专用持久化格式
...
[AgentFlowEditorShell.tsx](/home/taichu/git/1flowse/web/app/src/features/agent-flow/components/editor/AgentFlowEditorShell.tsx) 同时承担：
...
[AgentFlowCanvas.tsx](/home/taichu/git/1flowse/web/app/src/features/agent-flow/components/editor/AgentFlowCanvas.tsx) 里直接做：
...
[node-registry.tsx](/home/taichu/git/1flowse/web/app/src/features/agent-flow/components/nodes/node-registry.tsx) 目前同时做：
...
[NodeInspector.tsx](/home/taichu/git/1flowse/web/app/src/features/agent-flow/components/inspector/NodeInspector.tsx) 直接持有：
...
[default-agent-flow-document.ts](/home/taichu/git/1flowse/web/app/src/features/agent-flow/lib/default-agent-flow-document.ts) 同时放了：
...
相比“完整照搬 Dify 的实现细节”，该方案保留 1Flowse 现有 document schema、页面骨架和 API 合同，不会把当前 feature 强行改造成半套 Dify。
...
当前 1Flowse 还没有独立 custom connection line。  
重构后应补上，用于：
...
1Flowse 仍以更轻的 feature-local store 为主。

### 15.3 关键结果

重构后，1Flowse 会在“实现边界”上接近 Dify，而不是在“全部功能”和“全部状态量级”上强行追齐。
...
- 把 1Flowse 从“能编辑节点”升级为“有完整画布交互内核”
