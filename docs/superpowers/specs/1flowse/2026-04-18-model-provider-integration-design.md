...
- `../1flowse-official-plugins` 的模型供应商源码包结构
...
参考 `Dify` 已有的多语言 `README` 和内联 `I18nObject` 思路，`1Flowse` 进一步要求 provider plugin 拥有专门的 `i18n/` 目录，而不是把国际化文案散落在 `manifest / provider / model yaml` 中。
...
参考 `Dify` 已经具备的 `plugin init`、`module append` 和开发 `GUIDE`，`1Flowse` 还应额外提供“一键生成简单 demo 页面”的统一入口。
...
`1Flowse` 真正安装和运行的是 provider plugin 产物，而不是源码目录。
...
```text
1flowse@openai_compatible@0.1.0@<sha256>.1flowsepkg
```
...
6. 初始化 `../1flowse-official-plugins` 并落 `openai_compatible` 参考 plugin 源码、i18n 与 demo 脚手架
