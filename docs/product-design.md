...
> 文档分层约定：本文与 `docs/technical-design-supplement.md` 只保留产品 / 技术基线；开源项目定位与当前对外口径见 `docs/open-source-positioning.md`；当前实现索引放在 `docs/dev/`；当前开发者自己的按日期过程留痕如需保留，请放在 `docs/.private/history/`；废弃文档统一归档到 `docs/expired/`。
...
| Runtime Fact Layer | 由 `runs`、`node_runs`、`run_events`、artifacts 与相关 API 组成的统一运行基础事实层，先服务运行时，再向人、人与 AI 协作、AI 自治三类入口分发视图 |
| Run | 工作流一次执行实例 |
| Node Run | 某节点的一次执行实例 |
...
- 事实层先行：先建立独立运行基础事实层，再向工作台、发布接口、MCP 和 AI 自治入口分发观察与消费视图，而不是为不同入口各自维护一套状态。
- 双用户一致性：产品同时服务人和 AI，设计工作台、运行时、发布接口与追溯链路时，应保证人机交互、人与 AI 协作、AI 独立操作三种场景的结果语义保持一致。
- 结果与数据一致性：同一 run 在不同入口下的最终状态、结构化结果、artifact 引用、审批结论和恢复判断必须保持同源，不允许出现“人看到一套、AI 读到另一套”。
