...
| Runtime Fact Layer | 统一运行事实层，承载工作流执行、节点执行、事件与产物等基础记录，并向工作台、发布面与 AI 协作入口分发派生视图 |
| Run | 工作流一次执行实例 |
| Node Run | 某节点的一次执行实例 |
| Execution Class | 节点或工具的执行隔离级别，从轻执行、受控隔离到强隔离 |
| Sandbox | 为高风险执行提供隔离运行环境 |
| Sensitive Resource | 业务方声明的受控资源，平台负责访问流程与级别，不预置行业分类 |
| Access Request | 对受控资源发起的一次标准化访问申请 |
| Approval Ticket | 当访问需要人工审核时生成的待处理票据 |
| Skill | 服务侧托管的轻量技能文档，用于向 `llm_agent` 注入任务指引与参考资料 |
| Workflow Library / Starter Template | 用于承载创建入口、最佳实践骨架与团队可复用工作流模板的产品资产层 |
...
从产品视角看，`7Flows IR` 至少要覆盖以下几层：

- 工作流层
  - 一个 workflow 应能统一表达入口、节点、连线、变量与发布配置。
- 节点层
  - 每个 node 应同时具备业务语义、输入输出契约和运行策略。
  - 节点标准类型至少覆盖 `trigger`、`llm_agent`、`tool`、`sandbox_code`、`mcp_query`、`condition`、`router`、`loop`、`output`。
- 连线层
  - edge 既要表达控制流，也要表达数据流、条件和字段映射。
- 运行上下文层
  - runtime context 应包含静态输入、全局上下文、节点工作上下文、证据上下文和 artifact 引用。
- 运行事实层
  - 工作流执行记录、节点执行记录、事件时间线与运行产物共同构成统一事实源。
  - 调试视图、证据视图、发布详情与授权上下文都只能作为派生投影存在。
- 治理对象层
  - 敏感访问相关对象至少包括受控资源、访问申请与审批票据。
- 集成对象层
  - 对外开放面至少包括发布入口。
  - 服务侧技能资产至少包括技能正文与引用资料。

补充约束：

- `NodeType` 负责表达业务语义，例如 `llm_agent`、`tool`、`sandbox_code`。
- `execution class` 负责表达执行边界，不与节点类型绑定成一一对应关系。
- 同样是 `tool` 节点，也可因为来源、权限和风险不同而进入不同执行隔离级别。
- 工作流级事实、节点级事实、追加式事件时间线与运行产物引用应保持同源。
- 调试视图、证据视图、发布详情与授权上下文都必须从事实层派生；任何投影都不能反过来成为新的事实源。
...
- 开源默认轻执行：开源版优先保持低门槛可运行体验，普通 workflow 节点不以重隔离为开箱前提。
- 强隔离诚实失败：需要强隔离且不可安全降级的节点或工具，在没有可用隔离执行能力时应明确不可用，而不是静默降级。
...
- 全周期事实一致性：事实一致性不仅约束运行态，也应穿透开发、测试、部署与运行全周期；面向用户与市场场景定义的输入输出契约、验收样例、测试样例和发布行为应尽量保持同源，避免“测试里一套、线上一套、运营再解释一套”，以减少运行期 bug 并提升系统可维护性。
...
- 投影不是真相：给 MCP、发布接口、调试面板、发布治理页或运营入口使用的各种运行读模型都只能从事实层派生，不能各自维护旁路状态。
...
- 工作流编辑器：提供可视化画布、节点配置、变量配置与创建入口。
- 节点体系：覆盖 Trigger、LLM Agent、Tool、Sandbox Code、MCP Query、Condition/Router、Loop、Output 等核心节点。
- 工作流引擎：支持 DAG、条件分支、循环、重试、等待与恢复。
- 智能节点运行：支持多阶段智能节点、证据提炼、运行产物沉淀与结果追溯。
- 分级执行：可信能力保持轻量执行，高风险能力进入隔离执行。
- 模型与技能配置：一个智能节点可切换模型供应商，并按需注入服务侧技能资产。
- 节点权限模型：显式配置可读取的前序节点数据与上下文来源。
- MCP 上下文查询：节点可在运行时查询经授权的上下文和能力。
- 敏感访问控制：支持允许、拒绝、需审批或脱敏允许等决策，并接入等待、通知与恢复流程。
- 对外发布：支持平台原生接口、OpenAI 风格接口与 Anthropic 风格接口。
- 插件体系：支持 7Flows 原生插件与兼容层代理的注册、调用和结果映射。
- 工作流资产库：支持内置 starter、工作空间模板和后续生态模板来源分层，用于创建入口与复用沉淀。
- 基础运行调试：支持运行日志、节点输入输出、错误信息与流式事件查看。
...
- `OSS / Community`
  - 负责 `7Flows IR`、运行内核、基础追溯、可视化编排、自部署和开发者入口。
  - 默认保持低门槛可运行体验，并为隔离执行、兼容适配与治理能力预留标准扩展位。
...
- 编排与定义层：负责工作流、版本、节点配置、调试与发布管理。
- 编译与执行层：把设计态 `7Flows IR` 转为可运行蓝图，并统一驱动等待、恢复、重试、超时和状态流转。
- 智能节点运行层：负责主 AI、辅助提炼、上下文构建与结构化输出。
- 能力接入层：统一工具、模型、MCP、插件、隔离执行和兼容适配能力。
- 敏感访问与治理层：统一管理资源分级、访问申请、策略决策、审批、通知与恢复触发。
- 上下文与产物层：管理 global / working / evidence / artifact 四层上下文。
- 技能资产层：管理服务侧技能文档与引用资料，并按需注入智能节点。
- 发布与集成层：承接平台原生接口与兼容协议，并把请求路由到工作流。
- 观测与通知层：记录运行日志、流式事件、审计信息和指标，并向外发送通知。
...
- MCP 查询不绕过平台权限模型。
- MCP 返回的是 Runtime Fact Layer 上的“授权投影 + 内容引用”，而不是 UI 状态、发布缓存或全工作流原始内存快照。
- 支持动态查询和静态变量传递并存。
...
- 当前 `workflow_run_id`
- 当前 `node_id`
- 查询类型，例如 `authorized_context`
- 目标 source node 列表
- 允许读取的 artifact type 列表

### 7.5 授权链设计

- 工作流设计阶段配置节点级可读来源。
- 运行时生成授权上下文引用清单。
- MCP 只接受当前运行上下文内的合法查询。
- 审计日志记录“谁在何时读取了哪个节点产物”。
- 涉及敏感资源访问时，先发起标准化访问申请，再由统一敏感访问控制能力给出允许、拒绝、需审批或脱敏允许的决策。
- 若需要人工审批，运行时进入 `waiting`，生成审批票据并触发通知，批准后继续执行。
...
- `execution class` 负责描述执行边界，从轻执行、受控隔离到强隔离。
...
- 轻执行层
  - 面向可信内建能力与低风险调用，强调效率与低门槛体验。
- 隔离执行层
  - 面向代码、自定义能力、插件脚本、浏览器/文件类能力和其他高风险调用。
- 强隔离预留层
  - 面向极少数高权限或高合规要求场景，作为后续增强能力预留。

建议首版策略：

- 不是所有节点默认进入隔离执行。
- 高风险能力应默认进入隔离执行。
- 没有可用隔离能力时，不能把本应强隔离的能力默默降级为轻执行。
- 产品默认支持自托管优先的隔离执行接入，并为更强隔离能力预留扩展位。
- `sensitivity_level` 与 `execution class` 不相互替代：前者决定资源访问流程，后者决定执行边界。

### 8.3.1 OSS 默认与 Sandbox Backend 策略

- 社区 / 开源版本默认保持低门槛运行体验，不要求所有节点先配齐隔离环境。
- 平台应预留标准化隔离执行扩展位，便于社区、企业或官方提供不同强度的隔离能力。
- 隔离执行扩展位与兼容生态扩展位职责不同：前者解决执行边界，后者解决外部生态接入。
- 平台核心只关心隔离能力的声明、可用性与边界，不应被具体部署细节反向塑形。
...
- 原生平台层
  - 持有 7Flows 原生插件体系与统一能力入口
  - 不直接被某个外部生态的实现细节反向塑形
- 兼容适配层
  - 以可独立启停的方式接入 Dify 等外部生态
  - 负责生态能力转译、安装治理、运行治理与健康管理
...
- 7Flows 可以向智能节点提供服务侧 skill 文档，但真实桌面 / 本地环境操作仍由 OpenClaw 或本地助手执行；7Flows 不承担客户端接管或本地环境总控角色。
...
- 发布接口的同步结果、异步状态、流式事件和 detail 视图都必须从 Runtime Fact Layer 派生；缓存、导出或协议适配只做加速与重排，不成为新的事实源。
...
- 编排与创建面
  - 包含工作流编辑器、节点配置、变量配置、发布草稿与 starter 选择。
- 运行诊断面
  - 面向 run timeline、节点状态、错误、artifact、evidence 与 waiting 轨迹。
- 发布治理面
  - 面向 endpoint 配置、协议选择、鉴权、限流、缓存、调用活动与详情排障。
- 敏感访问治理面
  - 面向审批、通知、waiting / resume 与 operator 排障入口。
- 插件与凭证管理面
  - 面向插件生态、兼容层来源、凭证引用与调用治理。
- 资产复用面
  - 面向 workflow library、workspace starter 与模板治理。
...
- 调试面板、发布 detail、operator 入口和 AI 自治读模型应优先消费同一批统一运行事实投影，而不是每个入口自行拼装状态。
