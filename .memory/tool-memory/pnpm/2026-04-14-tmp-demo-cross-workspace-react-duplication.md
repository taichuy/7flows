...
match_when:
  ...
  - `pnpm --dir tmp/demo test` 在 `@1flowse/web` 中报 `Invalid hook call`
  - `@1flowse/ui` 已切到主仓库源码，但 demo 测试或构建出现双 React 症状
...
- `@1flowse/web` 测试内大量 `Invalid hook call`
- 栈里可见 `../../../web/node_modules/.pnpm/react...` 和 `../../../web/node_modules/.pnpm/antd...`

## 为什么会失败

- `tmp/demo` 与 `web/` 分别拥有自己的依赖树。
- 一旦把 `web/packages/*` 直接纳入 `tmp/demo` 的 workspace，`@1flowse/web` 在运行时会把来自 `web/node_modules` 的 `antd` 和 `react` 一起带进来。
